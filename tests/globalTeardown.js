...
module.exports = async () => {
    ...
    if (fs.existsSync(promptPidFile)) {
      ...
      if (ppid && !Number.isNaN(ppid)) {
        ...
        try {
          ...
          appendLog(
            `globalTeardown: prompt-mock kill SIGTERM failed: ${e && e.message}`
          );
        }
        try {
  ...
  try {
    try {
      ...
      if (rh && typeof rh.closeCachedServer === "function") {
        ...
        try {
          ...
          appendLog(
            `globalTeardown: closeCachedServer error: ${e && e.message}`
          );
        }
      }
      ...
      if (rh && typeof rh._restoreAndDestroySharedAgents === "function") {
        ...
        try {
          ...
          appendLog(
            `globalTeardown: _restoreAndDestroySharedAgents error: ${e && e.message}`
          );
        }
      }
      ...
      if (rh && typeof rh._forceCloseTemporaryServers === "function") {
        appendLog(
          "globalTeardown: force-closing temporary servers from request-helper"
        );
        try {
          ...
          appendLog(
            `globalTeardown: _forceCloseTemporaryServers error: ${e && e.message}`
          );
        }
      }
    } catch {}
    ...
    appendLog(
      `globalTeardown: closeCachedServer unexpected error: ${e && e.message}`
    );
  }
  ...
  try {
    try {
      ...
      if (app && typeof app.closeResources === "function") {
        ...
        try {
          ...
          appendLog(
            `globalTeardown: app.closeResources error: ${e && e.message}`
          );
        }
      }
    } catch (e) {
    ...
    appendLog(
      `globalTeardown: app cleanup unexpected error: ${e && e.message}`
    );
  }
  ...
  try {
    if (typeof process._getActiveHandles === "function") {
      ...
      for (let i = 0; i < handles.length; i++) {
        try {
          ...
          if (!name) continue;
          if (
            String(name) === "WriteStream" ||
            String(name) === "FileWriteStream"
          ) {
            try {
              ...
              if (p === childStdoutPath || p === childStderrPath) {
                try {
                  appendLog(
                    `globalTeardown: closing lingering write stream for ${p}`
                  );
                } catch {}
                try {
    ...
    appendLog(
      `globalTeardown: closing child stdout/stderr streams failed: ${e && e.message}`
    );
  }
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        appendLog(
          `globalTeardown: http.globalAgent.destroy failed: ${e && e.message}`
        );
      }
    }
    if (
      ...
      try {
        ...
        appendLog(
          `globalTeardown: https.globalAgent.destroy failed: ${e && e.message}`
        );
      }
    }
  } catch (e) {
      ...
      try {
        ...
        if (gd && typeof gd.close === "function") {
          try {
            gd.close();
            appendLog(
              "globalTeardown: undici.getGlobalDispatcher().close() called"
            );
          } catch (e) {
        ...
        if (gd && typeof gd.destroy === "function") {
          try {
            gd.destroy();
            appendLog(
              "globalTeardown: undici.getGlobalDispatcher().destroy() called"
            );
          } catch (e) {
            appendLog(
              `globalTeardown: undici.destroy failed: ${e && e.message}`
            );
          }
        }
      } catch {}
  ...
  try {
    ...
    if (Agent && typeof Agent.prototype.destroy === "function") {
      try {
        ...
        if (
          Agent.globalAgent &&
          typeof Agent.globalAgent.destroy === "function"
        ) {
        ...
        appendLog(
          `globalTeardown: Agent.globalAgent.destroy failed: ${e && e.message}`
        );
      }
    }
  } catch (e) {
    ...
    try {
      ...
      if (rh2 && typeof rh2._forceCloseTemporaryServers === "function") {
        ...
        try {
          ...
          appendLog(
            `globalTeardown: final _forceCloseTemporaryServers error: ${e && e.message}`
          );
        }
      }
    } catch {}
  ...
  try {
    ...
    const summary = handles
      .map((h) => h && h.constructor && h.constructor.name)
      .filter(Boolean);
    appendLog(
      `globalTeardown: activeHandles=${handles.length} types=${summary.join(",")}`
    );
  } catch (e) {
