...
module.exports = async () => {
  ...
  // After attempting to configure the prompt-mock child, if we expected
  // to spawn one but failed to set `PROMPT_URL`, fail fast so CI shows
  // a clear and actionable error (and uploads child stdout/stderr as
  // artifacts). This prevents tests from silently falling back to the
  // default `http://127.0.0.1:3000` and producing ECONNREFUSED noise.
  try {
    const expectedChild =
      process.env.USE_CHILD_PROCESS_SERVER === '1' || process.env.GITHUB_ACTIONS === 'true';
    const promptInfoFile = path.resolve(__dirname, 'prompt-mock.json');
    if (expectedChild && !process.env.PROMPT_URL && !fs.existsSync(promptInfoFile)) {
      const stdoutPath = path.resolve(__dirname, 'prompt-mock.child.stdout.log');
      const stderrPath = path.resolve(__dirname, 'prompt-mock.child.stderr.log');
      let out = '';
      let err = '';
      try {
        if (fs.existsSync(stdoutPath)) out = fs.readFileSync(stdoutPath, 'utf8');
      } catch (e) {
        out = `failed to read stdout: ${e && e.message}`;
      }
      try {
        if (fs.existsSync(stderrPath)) err = fs.readFileSync(stderrPath, 'utf8');
      } catch (e) {
        err = `failed to read stderr: ${e && e.message}`;
      }
      logLine('globalSetup: prompt-mock did not start; dumping prompt-mock stdout/stderr');
      logLine('PROMPT-MOCK STDOUT:\n' + out);
      logLine('PROMPT-MOCK STDERR:\n' + err);
      // Close the log stream to flush before throwing
      try {
        logStream.end();
      } catch {}
      throw new Error(
        'prompt-mock failed to start: see tests/prompt-mock.child.{stdout,stderr}.log'
      );
    }
  } catch (e) {
    // If reading diagnostics fails, ensure we still close the stream
    try {
      logStream.end();
    } catch {}
    throw e;
