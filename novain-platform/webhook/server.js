...
const DEBUG_TESTS =
  process.env.DEBUG_TESTS === "true" || process.env.DEBUG_TESTS === "1";
// ---- Config (env vars)
...
const fetchWithTimeout = async (url, opts = {}, ms = 60000) => {
  ...
  try {
    ...
    if (!IS_PROD || DEBUG_WEBHOOK) {
      ...
      if (bodyText)
        console.info("fetch response body:", bodyText.slice(0, 8000));
    }
    clearTimeout(id);
    return r;
  } catch (err) {
...
const SKIP_BODY_PARSER =
  ...
  process.env.SKIP_BODY_PARSER === "true";
if (SKIP_BODY_PARSER) {
  console.info(
    "SKIP_BODY_PARSER set — using lightweight JSON body parser (test mode)"
  );
...
app.use((req, _res, next) => {
  ...
  if (DEBUG_TESTS || (!IS_PROD && DEBUG_WEBHOOK)) {
    console.log(
      `[${new Date().toISOString()}] ${req.method} ${req.originalUrl} rid=${rid}`
    );
  }
  next();
});
...
try {
  const UNHANDLED_REJECTIONS_LOG =
    process.env.UNHANDLED_REJECTIONS_LOG ||
    path.join(os.tmpdir(), "vf_unhandled_rejections.log");
...
app.post("/export_lesson_file", (req, res) => {
  try {
    ...
    const safe =
      title.replace(/[^a-z0-9_-]+/gi, "_unused").slice(0, 64) || "lesson";
  ...
  try {
    ...
    if (action === "retrieve") {
      if (!RETRIEVAL_URL) {
        ...
        return res
          .status(400)
          .json({ ok: false, reply: "RETRIEVAL_URL_not_configured" });
      }
    ...
    if (action === "generate_lesson") {
      if (BUSINESS_URL) {
        try {
          ...
          console.warn(
            "generate_lesson: remote call failed, using stub fallback",
            e && e.message
          );
        }
      }
      ...
      try {
        if (PROMPT_URL) {
          if (DEBUG_TESTS) {
            try {
              console.info(
                "DEBUG_TESTS: llm_elicit: PROMPT_URL present:",
                !!PROMPT_URL
              );
              console.info(
                "DEBUG_TESTS: llm_elicit: fetchFn type:",
                typeof fetchFn
              );
              try {
          ...
          if (DEBUG_TESTS) {
            try {
              console.info(
                "DEBUG_TESTS: llm_elicit: fetched status:",
                r && r.status
              );
              try {
                const ct =
          ...
          if (!r.ok) {
            ...
            return res
              .status(502)
              .json({ ok: false, reply: "prompt_service_failed" });
          }
          ...
          if (payload.summary && String(payload.summary).trim()) {
            ...
            payload.promptLesson.strategySummary
          ) {
            ...
            if (candidates.length)
              summary = String(candidates[0]).slice(0, 800).trim();
          }
          ...
          if (!summary && payload.promptLesson) {
            ...
            if (Array.isArray(payload.promptLesson.demonstrationPrompts)) {
              payload.promptLesson.demonstrationPrompts
                .slice(0, 2)
                .forEach((p) => {
          ...
          const followup_question =
            payload.followup_question || payload.suggested_followup || "";
    ...
    if (action === "invoke_component") {
      ...
      if (comp === "C_CaptureQuestion") {
        ...
        const needs_clarify =
          qLower.includes("clarify") || qLower.includes("?");
        const followup_question = needs_clarify
      ...
      const rawPayload = {
        ...
        source: "invoke_component_default",
      };
      return res.status(200).json({
        ok: true,
        summary,
...
if (require.main === module) {
  ...
  const gracefulShutdown = (signal) => {
    ...
    try {
      ...
      try {
        console.error(
          "gracefulShutdown: server.close failed",
          e && e.stack ? e.stack : e
        );
      } catch {}
    }
  ...
  Object.defineProperty(app, "closeResources", {
    value: () => {
      ...
      try {
        if (
          sharedHttpsAgent &&
          typeof sharedHttpsAgent.destroy === "function"
        ) {
          try {
            sharedHttpsAgent.destroy();
          } catch {}
      ...
      try {
        ...
        if (
          fetchFn &&
          typeof fetchFn === "object" &&
          typeof fetchFn.close === "function"
        ) {
          try {
            fetchFn.close();
          } catch {}
