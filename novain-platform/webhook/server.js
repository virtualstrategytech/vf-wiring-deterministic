...
app.post('/export_lesson_file', (req, res) => {
  try {
    ...
    let lesson = {};
    try { lesson = typeof raw === 'string' ? JSON.parse(raw) : raw; } catch (_) {}
    const md = makeMarkdownFromLesson(title, lesson);
    const safe = title.replace(/[^a-z0-9_\-]+/gi, '_').slice(0,64) || 'lesson';
...
app.post('/webhook', async (req, res) => {
  ...
  // ✅ SAFE coercion starts here (replace your current lines with these)
  ...
  try {
    ...
    // ---- retrieve
    if (action === 'retrieve') {
      if (!RETRIEVAL_URL) return res.status(400).json({ ok: false, reply: 'RETRIEVAL_URL not configured' });
    ...
    if (action === 'generate_quiz') {
      ...
      const promptLesson = {
        ...
        demonstrationPrompts: [
          { label: "Single-shot", prompt: "You are a strategy coach. Using SPQA, rewrite this business question… [user’s question]. Output: a 4-step plan." },
          { label: "Few-shot",   prompt: "Here are 2 examples of good SPQA prompts… Now create one for: [user’s question]." },
          { label: "Refinement",  prompt: "Critique the following prompt for clarity, constraints, and measurability. Suggest a tighter version." }
        ],
      ...
      const quiz = {
        mcq: [
          { q: "In SPQA, what comes after Problem?", choices: ["Action","Question","Scope","Answer"], answer: "B", explain: "S → P → Q → A" },
          { q: "Best lever to reduce ambiguity fastest?", choices: ["More meetings","Answer top questions","Add stakeholders","Extend timeline"], answer: "B", explain: "Answering the right questions reduces uncertainty." },
          { q: "Which improves prompt reliability?", choices: ["Vague goals","No constraints","Explicit output format","Skip critique"], answer: "C", explain: "Specify structure & format." }
        ],
        tf: [
          { q: "SPQA stands for Situation, Problem, Question, Actions.", answer: true, explain: "Correct order." },
          { q: "Refinement/critique is optional for complex tasks.", answer: false, explain: "Refinement is essential for complex prompts." }
        ],
        open: [
          { q: "Rewrite the user’s question using SPQA. Provide one immediate 48-hour action.", rubric: ["Situation restated", "Problem measurable", "Top Qs listed", "48-hour action present"] }
        ]
      };
...
if (action === 'export_lesson') {
  try {
    ...
    // parse lesson JSON safely
    let lesson = {};
    try { lesson = typeof raw === 'string' ? JSON.parse(raw) : raw; } catch (e) { lesson = {}; }

    // simple markdown composer
    const md = makeMarkdownFromLesson(title, lesson);

    // base64 data URL (so you can "click to download" directly)
    ...
    console.error('export_lesson error:', e);
