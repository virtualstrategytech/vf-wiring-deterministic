...
app.post('/v1/teach-and-quiz', (req, res) => {
  ...
  const promptLesson = {
    ...
    demonstrationPrompts: [
      ...
      },
      {
        label: 'Refinement',
        prompt:
          'Critique the following prompt for clarity/constraints/metrics. Suggest a tighter version.',
      },
    ],
  ...
  const quiz = {
    mcq: [
      {
        q: 'In SPQA, what follows Problem?',
        choices: ['Action', 'Question', 'Scope', 'Answer'],
        answer: 'B',
        explain: 'S â†’ P â†’ Q â†’ A',
      },
      {
        q: 'Fastest way to reduce ambiguity?',
        choices: ['More meetings', 'Answer top questions', 'Add stakeholders', 'Extend timeline'],
        answer: 'B',
        explain: 'Answer key questions.',
      },
      {
        q: 'What improves prompt reliability?',
    ...
    tf: [
      {
        q: 'SPQA = Situation, Problem, Questions, Actions.',
        answer: true,
        explain: 'Correct order.',
      },
      {
        q: 'Refinement is optional for complex tasks.',
        answer: false,
        explain: 'Refinement is essential.',
      },
    ],
    open: [
      {
        q: 'Rewrite the userâ€™s question using SPQA. Provide one 48-hour action.',
        rubric: [
          'Situation restated',
          'Measurable problem',
          'Top Qs listed',
...
const server = app.listen(port, () => console.log('prompt-coach up on', port));

function gracefulShutdown(signal) {
  try {
    console.log(`Received ${signal}; shutting down prompt server`);
  } catch {}
  try {
