...
app.post('/v1/teach-and-quiz', (req, res) => {
  ...
  const promptLesson = {
    ...
    demonstrationPrompts: [
      {
        label: "Single-shot",
        prompt: `You are a strategy coach. Using SPQA, rewrite this business question: «${question}». Output a 4-step plan.`
      },
      {
        label: "Few-shot",
        prompt: `Here are two examples of strong SPQA prompts... Now create one for: «${question}».`
      },
      {
        label: "Refinement",
  ...
  const quiz = {
    mcq: [
      { q: "In SPQA, what follows Problem?", choices: ["Action","Question","Scope","Answer"], answer: "B", explain: "S → P → Q → A" },
    ...
    open: [
      { q: "Rewrite the user’s question using SPQA. Provide one 48-hour action.", rubric: ["Situation restated","Measurable problem","Top Qs listed","48-hour action present"] }
    ]
  };
